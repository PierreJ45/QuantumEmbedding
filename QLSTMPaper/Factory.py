...
class QLSTM(nn.Module):
    def __init__(
        ...
        self.wires_forget = list(range(0, n_qubits))
        self.wires_input = list(range(n_qubits, 2 * n_qubits))
        self.wires_update = list(range(2 * n_qubits, 3 * n_qubits))
        self.wires_output = list(range(3 * n_qubits, 4 * n_qubits))

        if self.backend == "qiskit.remote":
            self.dev_forget = qml.device(
                self.backend, wires=self.wires_forget, backend=backendObject
            )
            self.dev_input = qml.device(
                self.backend, wires=self.wires_input, backend=backendObject
            )
            self.dev_update = qml.device(
                self.backend, wires=self.wires_update, backend=backendObject
            )
            self.dev_output = qml.device(
                self.backend, wires=self.wires_output, backend=backendObject
            )
            
            print("initialized QLSTM with qiskit backend\n")
        else:
            self.dev_forget = qml.device(self.backend, wires=self.wires_forget)
            self.dev_input = qml.device(self.backend, wires=self.wires_input)
            self.dev_update = qml.device(self.backend, wires=self.wires_update)
            self.dev_output = qml.device(self.backend, wires=self.wires_output)
            print("initialized QLSTM with simulation\n")

        def ansatz(params, wires_type):
            # Entangling layer.
            # print('initialized Ansatz QLSTM\n')
            for i in range(1, 3):
        ...
        def VQC(features, weights, wires_type):
            ...
            # Flatten features if it is a 2D tensor
            if features.dim() > 1:
                features = features.flatten()

            # Create ry_params and rz_params correctly
            ry_params = [torch.arctan(feature) for feature in features]
            rz_params = [torch.arctan(feature**2) for feature in features]

            # Ensure lengths match
            assert (
                self.n_qubits == len(wires_type) == len(ry_params) == len(rz_params)
            ), "Mismatch in lengths"
    ...
    def forward(self, x, init_states=None):
        ...
        activation -> tanh
        """
        # print('running forward QLSTMezzzzz\n')
        ...
        # print('End of running QLSTM forward\n')
...
class QShallowRegressionLSTM(nn.Module):
    def __init__(
        ...
        print("initialized a QShallowLSTM\n")
        self.linear = nn.Linear(in_features=self.hidden_units, out_features=1)
    ...
    def forward(self, x):
        ...
        # print('running forward QShallowLSTM\n')
        return out
